...
# --- 5. PRE-PROCESSING & ANTI DUPLICATE ---
if df_dapot is not None and not df_dapot.empty:
    ...
    # 🔥 HAPUS BARIS DUPLIKAT DI DATABASE MASTER (1 ID = 1 BARIS)
...
if df_ume is not None and not df_ume.empty:
    ...
    # MASKING & EXTRACTING SPESIFIK ALARM
    s1_mask, cell_mask, vswr_mask, temp_mask = [pd.Series(False, index=df_ume.index)] * 4
    cond_pow, cond_l1, cond_l2, cond_pot = [pd.Series(False, index=df_ume.index)] * 4
    ...
    # 🔥 MENGHITUNG MURNI JUMLAH UNIK SITE TERDAMPAK (NUNIQUE)
    if df_dapot is not None and not df_dapot.empty:
        count_s1 = df_dapot.loc[df_dapot['C1'].isin(ids_s1) | df_dapot['C2'].isin(ids_s1) | df_dapot['C3'].isin(ids_s1), 'NE_CLEAN'].nunique()
        count_cell = df_dapot.loc[df_dapot['C1'].isin(ids_cell) | df_dapot['C2'].isin(ids_cell) | df_dapot['C3'].isin(ids_cell), 'NE_CLEAN'].nunique()
        count_vswr = df_dapot.loc[df_dapot['C1'].isin(ids_vswr) | df_dapot['C2'].isin(ids_vswr) | df_dapot['C3'].isin(ids_vswr), 'NE_CLEAN'].nunique()
        count_temp = df_dapot.loc[df_dapot['C1'].isin(ids_temp) | df_dapot['C2'].isin(ids_temp) | df_dapot['C3'].isin(ids_temp), 'NE_CLEAN'].nunique()
